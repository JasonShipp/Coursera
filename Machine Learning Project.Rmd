...
The aim of this project was to use data from accelerometers on the belt, forearm, arm and dumbell of 6 participants to train a machine learning algorithm in R to predict
the manner in which the participants carried out biceps curl exercises. This prediction model was then used to predict the exercise fashion of 20 different test cases, as carried
...
training<-(read.csv(file.path("C:/Users/Jason.shipp/Downloads/pml-training.csv"), stringsAsFactors=FALSE))
testing<- (read.csv(file.path("C:/Users/Jason.shipp/Downloads/pml-testing.csv"), stringsAsFactors=FALSE))
```  
...
* Predictors that correspond to zero information in the testing data were removed from the preprocessed training data:
...
* The following predictors showing close to zero variance were removed from the preprocessed training data:

```{r, echo=TRUE}
nearZeroVar(training_preprocessed, saveMetrics= TRUE)
```
```{r, echo=FALSE}
